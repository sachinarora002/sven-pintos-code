...
void
thread_initmore(void)
{
	initial_thread->pwd = dir_open_root();
	 //create the filesys thread
...
#ifdef USERPROG
  //initiated the frame table for virtual memory
  init_frame ();
#endif
               ...
               thread_func *function, void *aux) 
{
  ...
  t->me = (struct child *) malloc(sizeof(struct child));
  t->me->returnval = -1;
  t->me->pid = tid;
  t->me->alive = true;
  lock_init(&t->me->child_lock);
  cond_init (&t->me->child_condition);

  //add to the childrens list of the thread
  if(t != initial_thread) list_push_back(&thread_current()->children,&(t->me->parentelem));
...
void filesys_thread (void * dummy UNUSED) {
